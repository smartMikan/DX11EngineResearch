...
bool UserInterfaceClass::Initialize(D3DClass* Direct3D, int screenHeight, int screenWidth)
{
	...
	Sys_D3d = Direct3D;
	auto device = Sys_D3d->GetDevice();
	auto deviceContext = Sys_D3d->GetDeviceContext();
	...
	result = m_Font1->Initialize(device, L"./Resources/Font/format.txt", L"./Resources/Font/font.dds", 32.0f, 3);
	if (!result)
	{
		return false;
	}
	...
	result = m_FpsString->Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	// Initialize the fps text string.
	result = m_CpuString->Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	result = m_VideoStrings[0].Initialize(device, deviceContext, screenWidth, screenHeight, 256, false, m_Font1,
	...
	result = m_VideoStrings[1].Initialize(device, deviceContext, screenWidth, screenHeight, 32, false, m_Font1,
	...
	result = m_PositionStrings[0].Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	result = m_PositionStrings[1].Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	result = m_PositionStrings[2].Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	result = m_PositionStrings[3].Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	result = m_PositionStrings[4].Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	result = m_PositionStrings[5].Initialize(device, deviceContext, screenWidth, screenHeight, 16, false, m_Font1,
	...
	result = m_RenderCountStrings[0].Initialize(device, deviceContext, screenWidth, screenHeight, 32, false, m_Font1,
	...
	result = m_RenderCountStrings[1].Initialize(device, deviceContext, screenWidth, screenHeight, 32, false, m_Font1,
	...
	result = m_RenderCountStrings[2].Initialize(device, deviceContext, screenWidth, screenHeight, 32, false, m_Font1,
	...
	result = m_MiniMap->Initialize(device, deviceContext, screenWidth, screenHeight, 513, 513);
	if (!m_MiniMap)
	{
		return false;
	}
...
bool UserInterfaceClass::Frame(int fps,int cpu, float posX, float posY, float posZ,float rotX, float rotY, float rotZ)
{
	bool result;

	// Update the fps string.
	result = UpdateFpsString(fps);
	if (!result)
	{
		return false;
	}
	...
	result = UpdatePositionStrings(posX, posY, posZ, rotX, rotY, rotZ);
	if (!result)
	{
		return false;
	}
...
bool UserInterfaceClass::Render(ShaderManagerClass* ShaderManager, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX orthoMatrix)
{
	...
	Sys_D3d->TurnZBufferOff();
	Sys_D3d->TurnOnAlphaBlending();

	auto deviceContext = Sys_D3d->GetDeviceContext();

	// Render the fps string.
	m_FpsString->Render(deviceContext, ShaderManager, worldMatrix, viewMatrix, orthoMatrix, m_Font1->GetTexture());

	// Render the cpu string.
	m_CpuString->Render(deviceContext, ShaderManager, worldMatrix, viewMatrix, orthoMatrix, m_Font1->GetTexture());

	// Render the video card strings.
	m_VideoStrings[0].Render(deviceContext, ShaderManager, worldMatrix, viewMatrix, orthoMatrix, m_Font1->GetTexture());
	m_VideoStrings[1].Render(deviceContext, ShaderManager, worldMatrix, viewMatrix, orthoMatrix, m_Font1->GetTexture());

	// Render the position and rotation strings.
	for (i = 0; i<6; i++)
	{
		m_PositionStrings[i].Render(deviceContext, ShaderManager, worldMatrix, viewMatrix, orthoMatrix, m_Font1->GetTexture());
	}

	// Render the render count strings.
	for (i = 0; i < 3; i++)
	{
		m_RenderCountStrings[i].Render(deviceContext, ShaderManager, worldMatrix, viewMatrix, orthoMatrix, m_Font1->GetTexture());
	}

	// Turn off alpha blending now that the text has been rendered.
	Sys_D3d->TurnOffAlphaBlending();


	// Render the mini-map.
	result = m_MiniMap->Render(deviceContext, ShaderManager, worldMatrix, viewMatrix, orthoMatrix);
	if (!result)
	{
		return false;
	}
	...
	Sys_D3d->TurnZBufferOn();

	return true;
}

bool UserInterfaceClass::UpdateRenderCounts(int renderCount, int nodesDrawn, int nodesCulled)
{
	char tempString[32];
	char finalString[32];
	bool result;
	...
	result = m_RenderCountStrings[0].UpdateSentence(Sys_D3d->GetDeviceContext(), m_Font1, finalString, 10, 260, 1.0f, 1.0f, 1.0f);
	if (!result)
	{
		return false;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	result = m_RenderCountStrings[1].UpdateSentence(Sys_D3d->GetDeviceContext(), m_Font1, finalString, 10, 280, 1.0f, 1.0f, 1.0f);
	if (!result)
	{
		return false;
	}
	...
	// Update the sentence vertex buffer with the new string information.
	result = m_RenderCountStrings[2].UpdateSentence(Sys_D3d->GetDeviceContext(), m_Font1, finalString, 10, 300, 1.0f, 1.0f, 1.0f);
	if (!result)
	{
		return false;
	}
...
bool UserInterfaceClass::UpdateFpsString(int fps)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = m_FpsString->UpdateSentence(Sys_D3d->GetDeviceContext(), m_Font1, finalString, 10, 50, red, green, blue);
	if (!result)
	{
		return false;
	}
...
bool UserInterfaceClass::UpdateCpuString(int cpu)
{
	char tempString[16];
	char cpuString[16];
	float red, green, blue;
	...
	// Update the sentence vertex buffer with the new string information.
	result = m_CpuString->UpdateSentence(Sys_D3d->GetDeviceContext(), m_Font1, cpuString, 10, 70, red, green, blue);
	if (!result)
	{
		return false;
	}
...
bool UserInterfaceClass::UpdatePositionStrings(float posX, float posY, float posZ,
