...
bool TranslateShaderClass::Initialize(ID3D11Device *device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./Shader/TranslateVertexShader.hlsl", L"./Shader/TranslatePixelShader.hlsl");
	if (!result) {
		return false;
	}
	return true;
...
bool TranslateShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
...
void TranslateShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
