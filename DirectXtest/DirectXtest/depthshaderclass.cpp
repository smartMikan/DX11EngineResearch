...
bool DepthShaderClass::Initialize(ID3D11Device* device, HWND hwnd,ID3D11DeviceContext* deviceContext)
{
	...
	result = InitializeShader(device, hwnd, L"./Shader/depthVertexShader.hlsl", L"./Shader/depthPixelShader.hlsl");
	if(!result)
	{
		return false;
	}
...
bool DepthShaderClass::Render(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	bool result;
...
bool DepthShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	...
	result = result = D3DCompileFromFile(vsFilename, NULL, NULL, "main", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
