...
bool MultiTextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./Shader/AlphaMapVertexShader.hlsl", L"./Shader/AlphaMapPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
void MultiTextureShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* fileName)
{
	char* compileErrors;
